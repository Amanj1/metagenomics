...
for i in "${sorted_list_dir[@]}"; do
	...
	for j in "${sorted_ref[@]}"; do
		...
		$bbmap in1="${sorted_tmp_reads[0]}" in2="${sorted_tmp_reads[1]}" out="$paired_out" ref="$j"
		mv ref ref_paired
		samtools view -hbS "$paired_out" | samtools sort -O bam - | samtools flagstat - > "flagstat_""$paired_out"".txt" 		

		$bbmap in="${sorted_tmp_reads[2]}" ref="$j" out="$unpaired_out"
		samtools view -hbS "$paired_out" | samtools sort -O bam - | samtools flagstat - > "flagstat_""$unpaired_out"".txt"
		
		mv ref ref_unpaired
		mkdir -p $outname_dir
		mkdir $outname_ref
		mv "flagstat_""$unpaired_out"".txt" $outname_ref
		mv "flagstat_""$paired_out"".txt" $outname_ref
